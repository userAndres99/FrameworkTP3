...
export default function MapaInteractivoREST({
  onLocationSelect,
  center,
  tipoAnimal,
  readOnly = false,
  ...
  useEffect(() =>{
    async function fetchOrganizaciones(){
      try{
        const res = await fetch('/soap/organizaciones/rest', { headers: { Accept: 'application/json' } });
        if (!res.ok) throw new Error('Network response was not ok');
        const json = await res.json();
        const data = Array.isArray(json.data) ? json.data : [];
        console.log('Organizaciones REST recibidas:', data);
        setOrganizacion(data);
      }catch (err) {
        console.log('Error al cargar organizaciones REST', err);
      }
    }
    fetchOrganizaciones();
  },[]);
